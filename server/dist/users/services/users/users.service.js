    ...
    async create(user) {
        ...
        if (userDb) {
            if (userDb.email === user.email) {
                return { err: 'sign_email', msg: 'email già in uso' };
            }
            else {
                return { err: 'user', msg: 'username già in uso' };
            }
        }
        user.email = user.email.toLowerCase();
        let bcryptPass = await bcrypt.hash(user.pas, 10);
        let createUser = new this.userModel(Object.assign(Object.assign({}, user), { pas: bcryptPass }));
